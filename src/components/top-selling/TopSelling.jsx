...
const TopSelling = () => {
  ...
  useEffect(()=>{
    callApi(`http://localhost:3600/products?_limit=${initial_limit}`, {method: "get"})
  }, [])

  useEffect(()=>{
    if(topSellingQuery || limitQuery){
      callApi(`http://localhost:3600/products?_limit=${limitQuery}${searchParams.get("category") && `&category=${searchParams.get("category")}`}`, {method: "get"})
    }
  }, [searchParams])
